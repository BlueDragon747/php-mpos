...
class Statistics:
    ...
    def run(self, ctx: JobContext) -> None:
        ...
        interval = 180  # matches MPOS default

        # 1. Current pool hashrate (kH/s)
        try:
            hashrate = db.stats_current_hashrate(
        ...
        log.info("[%s] current pool hashrate: %.0f kH/s", self.name, hashrate)
        if cache:
        ...
        # 3. Per-user mining stats (recent-window hashrate)
        try:
