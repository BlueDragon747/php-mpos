...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    status|rotate|combine|rescan) ACTION="$1"; ACTION_EXPLICIT=1; shift ;;
...
pick_coins_and_toggles() {
  local raw coin index notice="" eof any la lb lc ld le lf desc _val
  local -A selected=()

  while :; do
    clear_tool_screen || true
    print_tool_banner force >&2
    printf '%sMAINNET WALLET MAINTENANCE for 25.2 Daemons%s\n' "$yellow" "$nc" >&2
    if [[ -n "$notice" ]]; then
    ...
    case "${raw,,}" in
      ...
      [1-6])
        ...
        ;;
      *)
        notice="enter 1-6, 0, A, B, C, D, E, F, or q"
        ;;
    esac
  done
}
