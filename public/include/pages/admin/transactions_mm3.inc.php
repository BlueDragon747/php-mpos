...
if (!$smarty->isCached('master.tpl', $smarty_cache_key)) {
  ...
  $aTransactions_mm3 = $transaction_mm3->getTransactions($start, @$_REQUEST['filter'], $iLimit);
  $aTransactionTypes = $transaction_mm3->getTypes();
  if (!$aTransactions_mm3) $_SESSION['POPUP'][] = array('CONTENT' => 'Could not find any transaction', 'TYPE' => 'errormsg');
  if (!$setting->getValue('disable_transactionsummary')) {
    $aTransactionSummary = $transaction_mm3->getTransactionSummary_mm3();
    $smarty->assign('SUMMARY', $aTransactionSummary);
  }
  $smarty->assign('LIMIT', $iLimit);
  $smarty->assign('TRANSACTIONS', $aTransactions_mm3);
