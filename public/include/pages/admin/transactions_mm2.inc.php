...
if (!$smarty->isCached('master.tpl', $smarty_cache_key)) {
  ...
  $aTransactions_mm2 = $transaction_mm2->getTransactions($start, @$_REQUEST['filter'], $iLimit);
  $aTransactionTypes = $transaction_mm2->getTypes();
  if (!$aTransactions_mm2) $_SESSION['POPUP'][] = array('CONTENT' => 'Could not find any transaction', 'TYPE' => 'errormsg');
  if (!$setting->getValue('disable_transactionsummary')) {
    $aTransactionSummary = $transaction_mm2->getTransactionSummary_mm2();
    $smarty->assign('SUMMARY', $aTransactionSummary);
  }
  $smarty->assign('LIMIT', $iLimit);
  $smarty->assign('TRANSACTIONS', $aTransactions_mm2);
