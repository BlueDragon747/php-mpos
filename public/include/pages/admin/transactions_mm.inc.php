...
if (!$smarty->isCached('master.tpl', $smarty_cache_key)) {
  ...
  $aTransactions_mm = $transaction_mm->getTransactions($start, @$_REQUEST['filter'], $iLimit);
  $aTransactionTypes = $transaction_mm->getTypes();
  if (!$aTransactions_mm) $_SESSION['POPUP'][] = array('CONTENT' => 'Could not find any transaction', 'TYPE' => 'errormsg');
  if (!$setting->getValue('disable_transactionsummary')) {
    $aTransactionSummary = $transaction_mm->getTransactionSummary_mm();
    $smarty->assign('SUMMARY', $aTransactionSummary);
  }
  $smarty->assign('LIMIT', $iLimit);
  $smarty->assign('TRANSACTIONS', $aTransactions_mm);
