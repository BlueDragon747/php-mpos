...
if (!$smarty->isCached('master.tpl', $smarty_cache_key)) {
  ...
  $aTransactions = $transaction_mm->getTransactions($start, @$_REQUEST['filter'], $iLimit);
  ...
  $aTransactionSummary = !$summary_disabled
    ? $transaction_mm->getTransactionSummary_mm()
    : null;

  $tx_v2_action          = 'transactions_mm';
  $tx_v2_page            = 'admin';
  $tx_v2_show_username   = true;
  $tx_v2_currency        = isset($config['currency_mm']) ? $config['currency_mm'] : '';
