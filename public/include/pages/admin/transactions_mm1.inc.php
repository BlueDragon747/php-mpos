...
if (!$smarty->isCached('master.tpl', $smarty_cache_key)) {
  ...
  $aTransactions_mm1 = $transaction_mm1->getTransactions($start, @$_REQUEST['filter'], $iLimit);
  $aTransactionTypes = $transaction_mm1->getTypes();
  if (!$aTransactions_mm1) $_SESSION['POPUP'][] = array('CONTENT' => 'Could not find any transaction', 'TYPE' => 'errormsg');
  if (!$setting->getValue('disable_transactionsummary')) {
    $aTransactionSummary = $transaction_mm1->getTransactionSummary_mm1();
    $smarty->assign('SUMMARY', $aTransactionSummary);
  }
  $smarty->assign('LIMIT', $iLimit);
  $smarty->assign('TRANSACTIONS', $aTransactions_mm1);
