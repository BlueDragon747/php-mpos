...
if (!$smarty->isCached('master.tpl', $smarty_cache_key)) {
  ...
  // Initialize defaults
  $aGetInfo_mm6 = array('errors' => 'Unable to connect');
  $dBalance_mm6 = 0;
  $dNewmint_mm6 = -1;

  try {
    if ($bitcoin_mm6->can_connect() === true){
      $dBalance_mm6 = $bitcoin_mm6->getbalance();
      $aGetInfo_mm6 = $bitcoin_mm6->getinfo();
      if (is_array($aGetInfo_mm6) && array_key_exists('newmint', $aGetInfo_mm6)) {
        $dNewmint_mm6 = $aGetInfo_mm6['newmint'];
      }
    } else {
  ...
  empty($config['network_confirmations_mm6']) ? $confirmations = 120 : $confirmations = $config['network_confirmations_mm6'];
  $aBlocksUnconfirmed_mm6 = $block_mm6->getAllUnconfirmed($confirmations);
  $dBlocksUnconfirmedBalance_mm6 = 0;
  if (!empty($aBlocksUnconfirmed_mm6))
    foreach ($aBlocksUnconfirmed_mm6 as $aData) $dBlocksUnconfirmedBalance_mm6 += $aData['amount'];

  // Fetch locked balance from transactions
  $dLockedBalance_mm6 = $transaction_mm6->getLockedBalance();

  // Cold wallet balance
  if (! $dColdCoins = $setting->getValue('wallet_cold_coins')) $dColdCoins = 0;
  $smarty->assign("UNCONFIRMED", $dBlocksUnconfirmedBalance_mm6);
  $smarty->assign("BALANCE", $dBalance_mm6);
