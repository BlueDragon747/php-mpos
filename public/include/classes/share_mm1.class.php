...
class Share_mm1 Extends Base {
  protected $table = 'shares_mm1';
  protected $tableArchive = 'shares_archive_mm1';
  private $oUpstream;
  ...
  public function moveArchive($current_upstream, $block_id, $previous_upstream=0) {
    if ($this->config['payout_system_mm1'] != 'pplns') {
...
$share_mm1 = new Share_mm1();
$share_mm1->setDebug($debug);
$share_mm1->setMysql($mysqli);
$share_mm1->setConfig($config);
$share_mm1->setUser($user);
$share_mm1->setBlock($block_mm1);
$share_mm1->setErrorCodes($aErrorCodes);
