...
class Transaction_mm5 extends Base {
  protected $table = 'transactions_mm5';
   ...
   **/
  public function getTransactionSummary_mm5($account_id=NULL) {
    if ($data = $this->memcache->get(__FUNCTION__ . $account_id)) return $data;
    $sql = "
      SELECT
        SUM(t.amount) AS total, t.type AS type
      FROM transactions_mm5 AS t
      LEFT OUTER JOIN blocks_mm5 AS b
      ON b.id = t.block_id
  ...
  public function getAPQueue() {
    ...
    $stmt = $this->mysqli->prepare("
      ...
      FROM $this->table AS t
      LEFT JOIN blocks_mm5 AS b
      ON t.block_id = b.id
      LEFT JOIN accounts AS a
      ON t.account_id = a.id
      WHERE t.archived = 0 AND a.ap_threshold_mm5 > 0 AND a.coin_address_mm5 IS NOT NULL AND a.coin_address_mm5 != ''
      GROUP BY t.account_id
      HAVING confirmed > a.ap_threshold_mm5 AND confirmed > " . $this->config['txfee_auto']);
    if ($this->checkStmt($stmt) && $stmt->execute() && $result = $stmt->get_result())
      return $result->fetch_all(MYSQLI_ASSOC);
    return $this->sqlError();
  }
  ...
  public function getMPQueue() {
    $stmt = $this->mysqli->prepare("
      ...
      LEFT JOIN " . $this->block->getTableName() . " AS b
      ON t.block_id = b.id
      WHERE p.completed = 0 AND t.archived = 0 AND a.coin_address_mm5 IS NOT NULL AND a.coin_address_mm5 != ''
      GROUP BY t.account_id
...
$transaction_mm5 = new Transaction_mm5();
$transaction_mm5->setMemcache($memcache);
$transaction_mm5->setNotification($notification);
$transaction_mm5->setDebug($debug);
$transaction_mm5->setMysql($mysqli);
$transaction_mm5->setConfig($config);
$transaction_mm5->setBlock($block_mm5);
$transaction_mm5->setUser($user);
$transaction_mm5->setPayout($oPayout);
$transaction_mm5->setErrorCodes($aErrorCodes);
?>
