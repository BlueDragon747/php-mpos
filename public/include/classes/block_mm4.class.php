...
class Block_mm4 extends Base {
  protected $table = 'blocks_mm4';
  ...
  public function getAverageAmount($limit=10) {
    $stmt = $this->mysqli->prepare("SELECT IFNULL(AVG(amount), " . $this->config['reward_mm4'] . ") as avg_amount FROM ( SELECT amount FROM $this->table ORDER BY id DESC LIMIT ?) AS t1");
    if ($this->checkStmt($stmt) && $stmt->bind_param('i', $limit) && $stmt->execute() && $result = $stmt->get_result()) {
...
$block_mm4 = new Block_mm4();
$block_mm4->setDebug($debug);
$block_mm4->setMysql($mysqli);
$block_mm4->setConfig($config);
$block_mm4->setErrorCodes($aErrorCodes);
