 ...
 **/
class Statistics_mm1 extends Base {
   ...
   * @return data array invalid and valid share counts
   **/
  public function getUserShares_mm1($username, $account_id=NULL) {
    $this->debug->append("STA " . __METHOD__, 4);
  ...
  public function getUserEstimates($value1, $value2, $dDonate, $bNoFees, $ppsvalue=0) {
    $this->debug->append("STA " . __METHOD__, 4);
    if ($this->config['payout_system_mm1'] != 'pps') {
      if (@$value1['valid'] > 0  && @$value2['valid'] > 0) {
        $this->config['reward_type'] == 'fixed' ? $reward = $this->config['reward_mm1'] : $reward = $this->block->getAverageAmount();
   ...
   */
  public function getNetworkExpectedTimePerBlock_mm1(){
    ...
    if ($data = $this->memcache->get($cacheKey)) {
      if ($data < 0) {
        $this->debug->append("WARNING: MM1 - Negative cached EstTimePerBlock detected: $data. Deleting cache.", 2);
    ...
    $this->debug->append("DEBUG: MM1 - Raw hashrate: " . var_export($dNetworkHashrate, true) . ", difficulty: " . var_export($dDifficulty, true), 3);
    
    // Validate hashrate is numeric and positive
    if (!is_numeric($dNetworkHashrate) || $dNetworkHashrate <= 0) {
      $this->debug->append("WARNING: MM1 - Invalid network hashrate: " . var_export($dNetworkHashrate, true) . ", using cointarget_mm1 fallback", 2);
      return $this->memcache->setCache($cacheKey, $this->config['cointarget_mm1']);
    }
    
    // Validate difficulty is numeric and positive
    if (!is_numeric($dDifficulty) || $dDifficulty <= 0) {
      $this->debug->append("WARNING: MM1 - Invalid difficulty: " . var_export($dDifficulty, true) . ", using cointarget_mm1 fallback", 2);
      return $this->memcache->setCache($cacheKey, $this->config['cointarget_mm1']);
    }
    ...
    $this->debug->append("DEBUG: MM1 - Calculated estTime: " . var_export($estTime, true), 3);
    
    // Validate the calculated estTime is numeric and positive before caching
    if (!is_numeric($estTime) || $estTime <= 0) {
      $this->debug->append("WARNING: MM1 - Invalid/negative EstTimePerBlock calculated: " . var_export($estTime, true) . ", using cointarget_mm1 fallback", 2);
      return $this->config['cointarget_mm1'];
    }
   ...
   **/
  public function getExpectedNextDifficulty_mm1(){
    $cacheKey = get_class($this) . __FUNCTION__ . '_' . md5(spl_object_hash($this->bitcoin));
    ...
    return $this->memcache->setCache($cacheKey, round($dDifficulty * $this->config['cointarget_mm1'] / $this->getNetworkExpectedTimePerBlock_mm1(), 8));
  }
   ...
   **/
  public function getBlocksUntilDiffChange_mm1(){
    $cacheKey = get_class($this) . __FUNCTION__ . '_' . md5(spl_object_hash($this->bitcoin));
    if ($data = $this->memcache->get($cacheKey)) return $data;
    ...
    if ($this->config['pps']['reward']['type'] == 'blockavg' && $this->block->getBlockCount() > 0) {
      ...
      if ($this->config['pps']['reward']['type'] == 'block') {
        if ($aLastBlock = $this->block->getLast()) {
          ...
          $pps_reward = $this->config['pps']['reward']['default_mm1'];
        }
      } else {
        $pps_reward = $this->config['pps']['reward']['default_mm1'];
      }
...
$statistics_mm1 = new Statistics_mm1();
$statistics_mm1->setDebug($debug);
$statistics_mm1->setMysql($mysqli);
$statistics_mm1->setShare($share_mm1);
$statistics_mm1->setUser($user);
$statistics_mm1->setBlock($block_mm1);
$statistics_mm1->setMemcache($memcache_mm1);
$statistics_mm1->setConfig($config);
$statistics_mm1->setBitcoin($bitcoin_mm1);
$statistics_mm1->setErrorCodes($aErrorCodes);
