...
class Block_mm2 extends Base {
  protected $table = 'blocks_mm2';
  ...
  public function getAverageAmount($limit=10) {
    $stmt = $this->mysqli->prepare("SELECT IFNULL(AVG(amount), " . $this->config['reward_mm2'] . ") as avg_amount FROM ( SELECT amount FROM $this->table ORDER BY id DESC LIMIT ?) AS t1");
    if ($this->checkStmt($stmt) && $stmt->bind_param('i', $limit) && $stmt->execute() && $result = $stmt->get_result()) {
...
$block_mm2 = new Block_mm2();
$block_mm2->setDebug($debug);
$block_mm2->setMysql($mysqli);
$block_mm2->setConfig($config);
$block_mm2->setErrorCodes($aErrorCodes);
