...
class Block_mm1 extends Base {
  protected $table = 'blocks_mm1';
  ...
  public function getAverageAmount($limit=10) {
    $stmt = $this->mysqli->prepare("SELECT IFNULL(AVG(amount), " . $this->config['reward_mm1'] . ") as avg_amount FROM ( SELECT amount FROM $this->table ORDER BY id DESC LIMIT ?) AS t1");
    if ($this->checkStmt($stmt) && $stmt->bind_param('i', $limit) && $stmt->execute() && $result = $stmt->get_result()) {
...
$block_mm1 = new Block_mm1();
$block_mm1->setDebug($debug);
$block_mm1->setMysql($mysqli);
$block_mm1->setConfig($config);
$block_mm1->setErrorCodes($aErrorCodes);
