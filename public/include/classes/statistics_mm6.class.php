 ...
 **/
class Statistics_mm6 extends Base {
   ...
   * @return data array invalid and valid share counts
   **/
  public function getUserShares_mm6($username, $account_id=NULL) {
    $this->debug->append("STA " . __METHOD__, 4);
  ...
  public function getUserEstimates($value1, $value2, $dDonate, $bNoFees, $ppsvalue=0) {
    $this->debug->append("STA " . __METHOD__, 4);
    if ($this->config['payout_system_mm6'] != 'pps') {
      if (@$value1['valid'] > 0  && @$value2['valid'] > 0) {
        $this->config['reward_type'] == 'fixed' ? $reward = $this->config['reward_mm6'] : $reward = $this->block->getAverageAmount();
   ...
   */
  public function getNetworkExpectedTimePerBlock_mm6(){
    if ($data = $this->memcache->get(get_class($this) . __FUNCTION__)) return $data;
    ...
    if($dNetworkHashrate <= 0){
      return $this->memcache->setCache(get_class($this) . __FUNCTION__, $this->config['cointarget_mm6']);
    }
   ...
   **/
  public function getExpectedNextDifficulty_mm6(){
    if ($data = $this->memcache->get(get_class($this) . __FUNCTION__)) return $data;
    ...
    return $this->memcache->setCache(get_class($this) . __FUNCTION__, round($dDifficulty * $this->config['cointarget_mm6'] / $this->getNetworkExpectedTimePerBlock_mm6(), 8));
  }
   ...
   **/
  public function getBlocksUntilDiffChange_mm6(){
    if ($data = $this->memcache->get(get_class($this) . __FUNCTION__)) return $data;
    ...
    if ($this->config['pps']['reward']['type'] == 'blockavg' && $this->block->getBlockCount() > 0) {
      ...
      if ($this->config['pps']['reward']['type'] == 'block') {
        if ($aLastBlock = $this->block->getLast()) {
          ...
          $pps_reward = $this->config['pps']['reward']['default_mm6'];
        }
      } else {
        $pps_reward = $this->config['pps']['reward']['default_mm6'];
      }
...
$statistics_mm6 = new Statistics_mm6();
$statistics_mm6->setDebug($debug);
$statistics_mm6->setMysql($mysqli);
$statistics_mm6->setShare($share_mm6);
$statistics_mm6->setUser($user);
$statistics_mm6->setBlock($block_mm6);
$statistics_mm6->setMemcache($memcache);
$statistics_mm6->setConfig($config);
$statistics_mm6->setBitcoin($bitcoin_mm6);
$statistics_mm6->setErrorCodes($aErrorCodes);
