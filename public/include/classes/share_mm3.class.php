...
class Share_mm3 Extends Base {
  protected $table = 'shares_mm3';
  protected $tableArchive = 'shares_archive_mm3';
  private $oUpstream;
  ...
  public function moveArchive($current_upstream, $block_id, $previous_upstream=0) {
    if ($this->config['payout_system_mm3'] != 'pplns') {
...
$share_mm3 = new Share_mm3();
$share_mm3->setDebug($debug);
$share_mm3->setMysql($mysqli);
$share_mm3->setConfig($config);
$share_mm3->setUser($user);
$share_mm3->setBlock($block_mm3);
$share_mm3->setErrorCodes($aErrorCodes);
