...
class Transaction_mm2 extends Base {
  protected $table = 'transactions_mm2';
   ...
   **/
  public function getTransactionSummary_mm2($account_id=NULL) {
    if ($data = $this->memcache->get(__FUNCTION__ . $account_id)) return $data;
    $sql = "
      SELECT
        SUM(t.amount) AS total, t.type AS type
      FROM transactions_mm2 AS t
      LEFT OUTER JOIN blocks_mm2 AS b
      ON b.id = t.block_id
  ...
  public function getAPQueue() {
    ...
    $stmt = $this->mysqli->prepare("
      ...
      FROM $this->table AS t
      LEFT JOIN blocks_mm2 AS b
      ON t.block_id = b.id
      LEFT JOIN accounts AS a
      ON t.account_id = a.id
      WHERE t.archived = 0 AND a.ap_threshold_mm2 > 0 AND a.coin_address_mm2 IS NOT NULL AND a.coin_address_mm2 != ''
      GROUP BY t.account_id
      HAVING confirmed > a.ap_threshold_mm2 AND confirmed > " . $this->config['txfee_auto']);
    if ($this->checkStmt($stmt) && $stmt->execute() && $result = $stmt->get_result())
      return $result->fetch_all(MYSQLI_ASSOC);
    return $this->sqlError();
  }
  ...
  public function getMPQueue() {
    $stmt = $this->mysqli->prepare("
      ...
      LEFT JOIN " . $this->block->getTableName() . " AS b
      ON t.block_id = b.id
      WHERE p.completed = 0 AND t.archived = 0 AND a.coin_address_mm2 IS NOT NULL AND a.coin_address_mm2 != ''
      GROUP BY t.account_id
...
$transaction_mm2 = new Transaction_mm2();
$transaction_mm2->setMemcache($memcache);
$transaction_mm2->setNotification($notification);
$transaction_mm2->setDebug($debug);
$transaction_mm2->setMysql($mysqli);
$transaction_mm2->setConfig($config);
$transaction_mm2->setBlock($block_mm2);
$transaction_mm2->setUser($user);
$transaction_mm2->setPayout($oPayout);
$transaction_mm2->setErrorCodes($aErrorCodes);
?>
