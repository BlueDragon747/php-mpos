 ...
 **/
class Statistics_mm3 extends Base {
   ...
   * @return data array invalid and valid share counts
   **/
  public function getUserShares_mm3($username, $account_id=NULL) {
    $this->debug->append("STA " . __METHOD__, 4);
  ...
  public function getUserEstimates($value1, $value2, $dDonate, $bNoFees, $ppsvalue=0) {
    $this->debug->append("STA " . __METHOD__, 4);
    if ($this->config['payout_system_mm3'] != 'pps') {
      if (@$value1['valid'] > 0  && @$value2['valid'] > 0) {
        $this->config['reward_type'] == 'fixed' ? $reward = $this->config['reward_mm3'] : $reward = $this->block->getAverageAmount();
   ...
   */
  public function getNetworkExpectedTimePerBlock_mm3(){
    ...
    if ($data = $this->memcache->get($cacheKey)) {
      if ($data < 0) {
        $this->debug->append("WARNING: MM3 - Negative cached EstTimePerBlock detected: $data. Deleting cache.", 2);
    ...
    $this->debug->append("DEBUG: MM3 - Raw hashrate: " . var_export($dNetworkHashrate, true) . ", difficulty: " . var_export($dDifficulty, true), 3);
    
    // Validate hashrate is numeric and positive
    if (!is_numeric($dNetworkHashrate) || $dNetworkHashrate <= 0) {
      $this->debug->append("WARNING: MM3 - Invalid network hashrate: " . var_export($dNetworkHashrate, true) . ", using cointarget_mm3 fallback", 2);
      return $this->memcache->setCache($cacheKey, $this->config['cointarget_mm3']);
    }
    
    // Validate difficulty is numeric and positive
    if (!is_numeric($dDifficulty) || $dDifficulty <= 0) {
      $this->debug->append("WARNING: MM3 - Invalid difficulty: " . var_export($dDifficulty, true) . ", using cointarget_mm3 fallback", 2);
      return $this->memcache->setCache($cacheKey, $this->config['cointarget_mm3']);
    }
    ...
    $this->debug->append("DEBUG: MM3 - Calculated estTime: " . var_export($estTime, true), 3);
    
    // Validate the calculated estTime is numeric and positive before caching
    if (!is_numeric($estTime) || $estTime <= 0) {
      $this->debug->append("WARNING: MM3 - Invalid/negative EstTimePerBlock calculated: " . var_export($estTime, true) . ", using cointarget_mm3 fallback", 2);
      return $this->config['cointarget_mm3'];
    }
   ...
   **/
  public function getExpectedNextDifficulty_mm3(){
    $cacheKey = get_class($this) . __FUNCTION__ . '_' . md5(spl_object_hash($this->bitcoin));
    ...
    return $this->memcache->setCache($cacheKey, round($dDifficulty * $this->config['cointarget_mm3'] / $this->getNetworkExpectedTimePerBlock_mm3(), 8));
  }
   ...
   **/
  public function getBlocksUntilDiffChange_mm3(){
    $cacheKey = get_class($this) . __FUNCTION__ . '_' . md5(spl_object_hash($this->bitcoin));
    if ($data = $this->memcache->get($cacheKey)) return $data;
    ...
    if ($this->config['pps']['reward']['type'] == 'blockavg' && $this->block->getBlockCount() > 0) {
      ...
      if ($this->config['pps']['reward']['type'] == 'block') {
        if ($aLastBlock = $this->block->getLast()) {
          ...
          $pps_reward = $this->config['pps']['reward']['default_mm3'];
        }
      } else {
        $pps_reward = $this->config['pps']['reward']['default_mm3'];
      }
...
$statistics_mm3 = new Statistics_mm3();
$statistics_mm3->setDebug($debug);
$statistics_mm3->setMysql($mysqli);
$statistics_mm3->setShare($share_mm3);
$statistics_mm3->setUser($user);
$statistics_mm3->setBlock($block_mm3);
$statistics_mm3->setMemcache($memcache_mm3);
$statistics_mm3->setConfig($config);
$statistics_mm3->setBitcoin($bitcoin_mm3);
$statistics_mm3->setErrorCodes($aErrorCodes);
