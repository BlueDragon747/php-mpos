...
class Transaction_mm1 extends Base {
  protected $table = 'transactions_mm1';
   ...
   **/
  public function getTransactionSummary_mm1($account_id=NULL) {
    if ($data = $this->memcache->get(__FUNCTION__ . $account_id)) return $data;
    $sql = "
      SELECT
        SUM(t.amount) AS total, t.type AS type
      FROM transactions_mm1 AS t
      LEFT OUTER JOIN blocks_mm1 AS b
      ON b.id = t.block_id
  ...
  public function getAPQueue() {
    ...
    $stmt = $this->mysqli->prepare("
      ...
      FROM $this->table AS t
      LEFT JOIN blocks_mm1 AS b
      ON t.block_id = b.id
      LEFT JOIN accounts AS a
      ON t.account_id = a.id
      WHERE t.archived = 0 AND a.ap_threshold_mm1 > 0 AND a.coin_address_mm1 IS NOT NULL AND a.coin_address_mm1 != ''
      GROUP BY t.account_id
      HAVING confirmed > a.ap_threshold_mm1 AND confirmed > " . $this->config['txfee_auto']);
    if ($this->checkStmt($stmt) && $stmt->execute() && $result = $stmt->get_result())
      return $result->fetch_all(MYSQLI_ASSOC);
    return $this->sqlError();
  }
  ...
  public function getMPQueue() {
    $stmt = $this->mysqli->prepare("
      ...
      LEFT JOIN " . $this->block->getTableName() . " AS b
      ON t.block_id = b.id
      WHERE p.completed = 0 AND t.archived = 0 AND a.coin_address_mm1 IS NOT NULL AND a.coin_address_mm1 != ''
      GROUP BY t.account_id
...
$transaction_mm1 = new Transaction_mm1();
$transaction_mm1->setMemcache($memcache);
$transaction_mm1->setNotification($notification);
$transaction_mm1->setDebug($debug);
$transaction_mm1->setMysql($mysqli);
$transaction_mm1->setConfig($config);
$transaction_mm1->setBlock($block_mm1);
$transaction_mm1->setUser($user);
$transaction_mm1->setPayout($oPayout);
$transaction_mm1->setErrorCodes($aErrorCodes);
?>
