 ...
 **/
class Statistics_mm4 extends Base {
   ...
   * @return data array invalid and valid share counts
   **/
  public function getUserShares_mm4($username, $account_id=NULL) {
    $this->debug->append("STA " . __METHOD__, 4);
  ...
  public function getUserEstimates($value1, $value2, $dDonate, $bNoFees, $ppsvalue=0) {
    $this->debug->append("STA " . __METHOD__, 4);
    if ($this->config['payout_system_mm4'] != 'pps') {
      if (@$value1['valid'] > 0  && @$value2['valid'] > 0) {
        $this->config['reward_type'] == 'fixed' ? $reward = $this->config['reward_mm4'] : $reward = $this->block->getAverageAmount();
  ...
  public function getNetworkExpectedTimePerBlock_mm4(){
    ...
    if ($data = $this->memcache->get($cacheKey)) {
      if ($data < 0) {
        $this->debug->append("WARNING: MM4 - Negative cached EstTimePerBlock detected: $data. Deleting cache.", 2);
    ...
    if (!is_numeric($estTime) || $estTime <= 0) {
      ...
      return $this->config['cointarget_mm4'];
    }
  ...
  public function getExpectedNextDifficulty_mm4(){
    ...
    return $this->memcache->setCache($cacheKey, round($dDifficulty * $this->config['cointarget_mm4'] / $this->getNetworkExpectedTimePerBlock_mm4(), 8));
  }
    ...
    if ($this->config['pps']['reward']['type'] == 'blockavg' && $this->block->getBlockCount() > 0) {
      ...
      if ($this->config['pps']['reward']['type'] == 'block') {
        if ($aLastBlock = $this->block->getLast()) {
          ...
          $pps_reward = $this->config['pps']['reward']['default_mm4'];
        }
      } else {
        $pps_reward = $this->config['pps']['reward']['default_mm4'];
      }
...
$statistics_mm4 = new Statistics_mm4();
$statistics_mm4->setDebug($debug);
$statistics_mm4->setMysql($mysqli);
