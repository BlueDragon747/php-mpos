 ...
 **/
class Statistics_mm2 extends Base {
   ...
   * @return data array invalid and valid share counts
   **/
  public function getUserShares_mm2($username, $account_id=NULL) {
    $this->debug->append("STA " . __METHOD__, 4);
  ...
  public function getUserEstimates($value1, $value2, $dDonate, $bNoFees, $ppsvalue=0) {
    $this->debug->append("STA " . __METHOD__, 4);
    if ($this->config['payout_system_mm2'] != 'pps') {
      if (@$value1['valid'] > 0  && @$value2['valid'] > 0) {
        $this->config['reward_type'] == 'fixed' ? $reward = $this->config['reward_mm2'] : $reward = $this->block->getAverageAmount();
   ...
   */
  public function getNetworkExpectedTimePerBlock_mm2(){
    if ($data = $this->memcache->get(get_class($this) . __FUNCTION__)) return $data;
    ...
    if($dNetworkHashrate <= 0){
      return $this->memcache->setCache(get_class($this) . __FUNCTION__, $this->config['cointarget_mm2']);
    }

    return $this->memcache->setCache(get_class($this) . __FUNCTION__, pow(2, 32) * $dDifficulty / $dNetworkHashrate);
  }
   ...
   **/
  public function getExpectedNextDifficulty_mm2(){
    if ($data = $this->memcache->get(get_class($this) . __FUNCTION__)) return $data;
    ...
    return $this->memcache->setCache(get_class($this) . __FUNCTION__, round($dDifficulty * $this->config['cointarget_mm2'] / $this->getNetworkExpectedTimePerBlock_mm2(), 8));
  }
   ...
   **/
  public function getBlocksUntilDiffChange_mm2(){
    if ($data = $this->memcache->get(get_class($this) . __FUNCTION__)) return $data;
    ...
    return $this->memcache->setCache(get_class($this) . __FUNCTION__, $this->config['coindiffchangetarget'] - ($iBlockcount % $this->config['coindiffchangetarget']));
  }
    ...
    if ($this->config['pps']['reward']['type'] == 'blockavg' && $this->block->getBlockCount() > 0) {
      ...
      if ($this->config['pps']['reward']['type'] == 'block') {
        if ($aLastBlock = $this->block->getLast()) {
          ...
          $pps_reward = $this->config['pps']['reward']['default_mm2'];
        }
      } else {
        $pps_reward = $this->config['pps']['reward']['default_mm2'];
      }
...
$statistics_mm2 = new Statistics_mm2();
$statistics_mm2->setDebug($debug);
$statistics_mm2->setMysql($mysqli);
$statistics_mm2->setShare($share_mm2);
$statistics_mm2->setUser($user);
$statistics_mm2->setBlock($block_mm2);
$statistics_mm2->setMemcache($memcache);
$statistics_mm2->setConfig($config);
$statistics_mm2->setBitcoin($bitcoin_mm2);
$statistics_mm2->setErrorCodes($aErrorCodes);
