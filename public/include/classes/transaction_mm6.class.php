...
class Transaction_mm6 extends Base {
  protected $table = 'transactions_mm6';
   ...
   **/
  public function getTransactionSummary_mm6($account_id=NULL) {
    if ($data = $this->memcache->get(__FUNCTION__ . $account_id)) return $data;
    $sql = "
      SELECT
        SUM(t.amount) AS total, t.type AS type
      FROM transactions_mm6 AS t
      LEFT OUTER JOIN blocks_mm6 AS b
      ON b.id = t.block_id
  ...
  public function getAPQueue() {
    ...
    $stmt = $this->mysqli->prepare("
      ...
      FROM $this->table AS t
      LEFT JOIN blocks_mm6 AS b
      ON t.block_id = b.id
      LEFT JOIN accounts AS a
      ON t.account_id = a.id
      WHERE t.archived = 0 AND a.ap_threshold_mm6 > 0 AND a.coin_address_mm6 IS NOT NULL AND a.coin_address_mm6 != ''
      GROUP BY t.account_id
      HAVING confirmed > a.ap_threshold_mm6 AND confirmed > " . $this->config['txfee_auto']);
    if ($this->checkStmt($stmt) && $stmt->execute() && $result = $stmt->get_result())
      return $result->fetch_all(MYSQLI_ASSOC);
    return $this->sqlError();
  }
  ...
  public function getMPQueue() {
    $stmt = $this->mysqli->prepare("
      ...
      LEFT JOIN " . $this->block->getTableName() . " AS b
      ON t.block_id = b.id
      WHERE p.completed = 0 AND t.archived = 0 AND a.coin_address_mm6 IS NOT NULL AND a.coin_address_mm6 != ''
      GROUP BY t.account_id
...
$transaction_mm6 = new Transaction_mm6();
$transaction_mm6->setMemcache($memcache);
$transaction_mm6->setNotification($notification);
$transaction_mm6->setDebug($debug);
$transaction_mm6->setMysql($mysqli);
$transaction_mm6->setConfig($config);
$transaction_mm6->setBlock($block_mm4);
$transaction_mm6->setUser($user);
$transaction_mm6->setPayout($oPayout);
$transaction_mm6->setErrorCodes($aErrorCodes);
?>
