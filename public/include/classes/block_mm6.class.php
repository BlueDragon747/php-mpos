...
class Block_mm6 extends Base {
  protected $table = 'blocks_mm6';
  ...
  public function getAverageAmount($limit=10) {
    $stmt = $this->mysqli->prepare("SELECT IFNULL(AVG(amount), " . $this->config['reward_mm6'] . ") as avg_amount FROM ( SELECT amount FROM $this->table ORDER BY id DESC LIMIT ?) AS t1");
    if ($this->checkStmt($stmt) && $stmt->bind_param('i', $limit) && $stmt->execute() && $result = $stmt->get_result()) {
...
$block_mm6 = new Block_mm6();
$block_mm6->setDebug($debug);
$block_mm6->setMysql($mysqli);
$block_mm6->setConfig($config);
$block_mm6->setErrorCodes($aErrorCodes);
