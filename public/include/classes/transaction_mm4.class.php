...
class Transaction_mm4 extends Base {
  protected $table = 'transactions_mm4';
   ...
   **/
  public function getTransactionSummary_mm4($account_id=NULL) {
    if ($data = $this->memcache->get(__FUNCTION__ . $account_id)) return $data;
    $sql = "
      SELECT
        SUM(t.amount) AS total, t.type AS type
      FROM transactions_mm4 AS t
      LEFT OUTER JOIN blocks_mm4 AS b
      ON b.id = t.block_id
  ...
  public function getAPQueue() {
    ...
    $stmt = $this->mysqli->prepare("
      ...
      FROM $this->table AS t
      LEFT JOIN blocks_mm4 AS b
      ON t.block_id = b.id
      LEFT JOIN accounts AS a
      ON t.account_id = a.id
      WHERE t.archived = 0 AND a.ap_threshold_mm4 > 0 AND a.coin_address_mm4 IS NOT NULL AND a.coin_address_mm4 != ''
      GROUP BY t.account_id
      HAVING confirmed > a.ap_threshold_mm4 AND confirmed > " . $this->config['txfee_auto']);
    if ($this->checkStmt($stmt) && $stmt->execute() && $result = $stmt->get_result())
      return $result->fetch_all(MYSQLI_ASSOC);
    return $this->sqlError();
  }
  ...
  public function getMPQueue() {
    $stmt = $this->mysqli->prepare("
      ...
      LEFT JOIN " . $this->block->getTableName() . " AS b
      ON t.block_id = b.id
      WHERE p.completed = 0 AND t.archived = 0 AND a.coin_address_mm4 IS NOT NULL AND a.coin_address_mm4 != ''
      GROUP BY t.account_id
...
$transaction_mm4 = new Transaction_mm4();
$transaction_mm4->setMemcache($memcache);
$transaction_mm4->setNotification($notification);
$transaction_mm4->setDebug($debug);
$transaction_mm4->setMysql($mysqli);
$transaction_mm4->setConfig($config);
$transaction_mm4->setBlock($block_mm4);
$transaction_mm4->setUser($user);
$transaction_mm4->setPayout($oPayout);
$transaction_mm4->setErrorCodes($aErrorCodes);
?>
