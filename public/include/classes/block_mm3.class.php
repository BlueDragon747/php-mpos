...
class Block_mm3 extends Base {
  protected $table = 'blocks_mm3';
  ...
  public function getAverageAmount($limit=10) {
    $stmt = $this->mysqli->prepare("SELECT IFNULL(AVG(amount), " . $this->config['reward_mm3'] . ") as avg_amount FROM ( SELECT amount FROM $this->table ORDER BY id DESC LIMIT ?) AS t1");
    if ($this->checkStmt($stmt) && $stmt->bind_param('i', $limit) && $stmt->execute() && $result = $stmt->get_result()) {
...
$block_mm3 = new Block_mm3();
$block_mm3->setDebug($debug);
$block_mm3->setMysql($mysqli);
$block_mm3->setConfig($config);
$block_mm3->setErrorCodes($aErrorCodes);
