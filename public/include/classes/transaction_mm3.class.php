...
class Transaction_mm3 extends Base {
  protected $table = 'transactions_mm3';
   ...
   **/
  public function getTransactionSummary_mm3($account_id=NULL) {
    if ($data = $this->memcache->get(__FUNCTION__ . $account_id)) return $data;
    $sql = "
      SELECT
        SUM(t.amount) AS total, t.type AS type
      FROM transactions_mm3 AS t
      LEFT OUTER JOIN blocks_mm3 AS b
      ON b.id = t.block_id
  ...
  public function getAPQueue() {
    ...
    $stmt = $this->mysqli->prepare("
      ...
      FROM $this->table AS t
      LEFT JOIN blocks_mm3 AS b
      ON t.block_id = b.id
      LEFT JOIN accounts AS a
      ON t.account_id = a.id
      WHERE t.archived = 0 AND a.ap_threshold_mm3 > 0 AND a.coin_address_mm3 IS NOT NULL AND a.coin_address_mm3 != ''
      GROUP BY t.account_id
      HAVING confirmed > a.ap_threshold_mm3 AND confirmed > " . $this->config['txfee_auto']);
    if ($this->checkStmt($stmt) && $stmt->execute() && $result = $stmt->get_result())
      return $result->fetch_all(MYSQLI_ASSOC);
    return $this->sqlError();
  }
  ...
  public function getMPQueue() {
    $stmt = $this->mysqli->prepare("
      ...
      LEFT JOIN " . $this->block->getTableName() . " AS b
      ON t.block_id = b.id
      WHERE p.completed = 0 AND t.archived = 0 AND a.coin_address_mm3 IS NOT NULL AND a.coin_address_mm3 != ''
      GROUP BY t.account_id
...
$transaction_mm3 = new Transaction_mm3();
$transaction_mm3->setMemcache($memcache);
$transaction_mm3->setNotification($notification);
$transaction_mm3->setDebug($debug);
$transaction_mm3->setMysql($mysqli);
$transaction_mm3->setConfig($config);
$transaction_mm3->setBlock($block_mm3);
$transaction_mm3->setUser($user);
$transaction_mm3->setPayout($oPayout);
$transaction_mm3->setErrorCodes($aErrorCodes);
?>
