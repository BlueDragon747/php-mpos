...
class Share_mm2 Extends Base {
  protected $table = 'shares_mm2';
  protected $tableArchive = 'shares_archive_mm2';
  private $oUpstream;
  ...
  public function moveArchive($current_upstream, $block_id, $previous_upstream=0) {
    if ($this->config['payout_system_mm2'] != 'pplns') {
...
$share_mm2 = new Share_mm2();
$share_mm2->setDebug($debug);
$share_mm2->setMysql($mysqli);
$share_mm2->setConfig($config);
$share_mm2->setUser($user);
$share_mm2->setBlock($block_mm2);
$share_mm2->setErrorCodes($aErrorCodes);
