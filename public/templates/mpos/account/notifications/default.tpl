...
{else}
  {if $N_CSS}
    {foreach from=$N_CSS item=cssPath}
      <link rel="stylesheet" href="{$cssPath}">
    {/foreach}
  {/if}
  <div id="bsx-v2-shell">
    <div id="app-notifications"
         data-initial='{$N_INITIAL_JSON nofilter}'></div>
  </div>
  {if $N_JS}
    <script type="module" src="{$N_JS}"></script>
  {else}
...
<style>
  ...
  [data-theme="light"] #bsx-v2-shell .setting-label { color: #2d3748; }

  [data-theme="light"] #bsx-v2-shell .history-table thead th {
    color: #4a5568;
    background: #f1f3f5;
    border-bottom-color: rgba(0, 0, 0, 0.10);
  }
  [data-theme="light"] #bsx-v2-shell .history-table th,
  [data-theme="light"] #bsx-v2-shell .history-table td {
    border-bottom-color: rgba(0, 0, 0, 0.06);
    color: #2d3748;
  }
  [data-theme="light"] #bsx-v2-shell .history-table tbody tr:nth-child(even) td {
