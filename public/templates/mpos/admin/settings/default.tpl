<style>
  ...
  /* !important overrides jQuery UI's theme rules that ship the tab
     strip with a framing box. */
  ...
  /* !important required: jQuery UI's .ui-tabs-anchor rules have
     higher specificity than these scoped selectors. */
  ...
  /* !important: dark-mode rule above ships !important so the
     light-mode override must match its specificity. */
  ...
  .settings-page .settings-inline-toggle {
    ...
    align-items: center;
    gap: 4px;
    /* pointer-events: none on the wrapper; children opt back in so
       only the <select> is clickable. */
    pointer-events: none;
  }
...
{foreach item=TAB from=array_keys($SETTINGS)}
        <li><a href="#{$TAB}">{$SETTINGS_TAB_LABELS[$TAB]|default:$TAB|capitalize}</a></li>
{/foreach}
      </ul>
    </header>
